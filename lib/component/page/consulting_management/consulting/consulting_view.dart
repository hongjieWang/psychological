...
class _ConsultingPageState extends State<ConsultingPage> {
  ...
  Future<void> _handlerRefresh() async {
    _pageNum = 1;
    loading = true;
    ApiService().getConsultCounselorStay(_pageNum).then((value) => {
          setState(() {
            _consultList.clear();
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          actions: [
            TextButton(
                onPressed: () {
                  Get.toNamed(RouteConfig.consultingAllPage);
                },
                child: Text(
                  "全部",
  ...
  bool onBtnStatus(item) {
    if (item['status'] == AppConstant.APPOINTMENT_NO) return false;
    return item['status'] == AppConstant.SIGN_IN ||
        timeLessAbs(
  ...
  Widget _appointmentBtn(item) {
    return Container(
      ...
      child: TextButton(
        onPressed: () {
          onBtnStatus(item) ? _onClickStart(item) : null;
        },
        child: Text(
        ...
        style: ButtonStyle(
            backgroundColor: MaterialStateProperty.resolveWith((states) {
              //默认不使用背景颜色
              return onBtnStatus(item) ? Color(0xFF5694E9) : Colors.black26;
            }),
      ...
      child: Row(
        children: [
          InkWell(
            ...
            child: Text(
                item['membersName'] == null ? '匿名' : item['membersName'],
                style:
